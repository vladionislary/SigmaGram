...
import ua.itaysonlab.sigmagram.preferences.ktx.category
import ua.itaysonlab.sigmagram.preferences.ktx.textDetail
import ua.itaysonlab.sigmagram.preferences.ktx.textIcon
import ua.itaysonlab.sigmagram.preferences.ktx.tgKitScreen
...
class MainPreferencesEntry : BasePreferencesEntry {
        ...
        category(LocaleController.getString("AS_Header_Categories", R.string.AS_Header_Categories)) {
            textIcon {
                ...
                listener = TGKitTextIconRow.TGTIListener {
                    it.presentFragment(SigmaGramPreferencesNavigator.createAppearance())
                }
            }
            ...
            textIcon {
                ...
                listener = TGKitTextIconRow.TGTIListener {
                    it.presentFragment(SigmaGramPreferencesNavigator.createChats())
                }
            }
            ...
            if (DoubleBottomBridge.isDbConfigAvailable()) {
                textIcon {
                    ...
                    listener = TGKitTextIconRow.TGTIListener {
                        it.presentFragment(SigmaGramPreferencesNavigator.createDB())
                    }
                }
            }
            ...
            textIcon {
                ...
                icon = R.drawable.menu_secret
                listener = TGKitTextIconRow.TGTIListener {
                    it.presentFragment(SigmaGramPreferencesNavigator.createSecurity())
                }
            }
            textIcon {
                ...
                listener = TGKitTextIconRow.TGTIListener {
                    it.presentFragment(SigmaGramPreferencesNavigator.createUpdates())
                }
            }
            textIcon {
                ...
                listener = TGKitTextIconRow.TGTIListener {
                    it.presentFragment(SigmaGramPreferencesNavigator.createDonate())
                }
            }

            category(LocaleController.getString("AS_Header_About", R.string.AS_Header_About)) {
                textDetail {
                    title = "SigmaGram " + SigmaGramExtras.CG_VERSION + " [" + BuildVars.BUILD_VERSION_STRING + "]"
    ...
    companion object {
        private fun goToChannel(bf: BaseFragment) {
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://t.me/sigmagram"))
        ...
        private fun goToChat(bf: BaseFragment) {
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://t.me/sigmagram_en"))
            val componentName = ComponentName(bf.parentActivity.packageName, LaunchActivity::class.java.name)
            intent.component = componentName
            intent.putExtra(Browser.EXTRA_CREATE_NEW_TAB, true)
            intent.putExtra(Browser.EXTRA_APPLICATION_ID, bf.parentActivity.packageName)
        ...
        private fun goToCrowdin(bf: BaseFragment) {
            ...
            openURL.data = Uri.parse("https://crowdin.com/project/sigmagram")
            bf.parentActivity.startActivity(openURL)
        }

        private fun goToGithub(bf: BaseFragment) {
            val openURL = Intent(android.content.Intent.ACTION_VIEW)
            openURL.data = Uri.parse("https://github.com/SigmaGram/SigmaGram")
            bf.parentActivity.startActivity(openURL)
        }
